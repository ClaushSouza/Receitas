...
export default function Carnes({ navigation }) {
  ...
  const imagePick = async () => {
    ...
    if (!result.cancelled) {
      ...
      const refs = ref(storage, `Carnes/${timeimg}`);
  ...
  //Trazer a Imagem do Banco

  useEffect(() => {
    const pushImg = async() => {
      const storage = getStorage();
      const reference = ref(storage, 'Carnes/1664324343246');
      await getDownloadURL(reference).then((x) => {
        setUrl(x);
      })
    }

    if(url == undefined) {pushImg()};
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <View style={styles.modal}>
                    ...
                    <Text style={styles.description}>{item.nome}</Text>
                      <Image style={{width: '100%', height: 250, alignItems: 'center', borderColor: 'black', borderWidth: 5, borderRadius: 5}} source={{uri: url}}/>
